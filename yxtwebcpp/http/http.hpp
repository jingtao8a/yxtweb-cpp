...
bool checkGetAs(const MapType& m, const std::string& key, T& val, const T& def = T()) {//带检查的checkas,返回false代表获取失败
    auto it = m.find(key);
    if(it == m.end()) {
        val = def;
        return false;
...
private:
    ...
    uint8_t m_version;
    /// 是否自动关闭
    bool m_close;//connection参数：keep_alive或close
    /// 是否为websocket
    bool m_websocket;//是否使用websocket协议

    uint8_t m_parserParamFlag;//访问参数的flag
    /// 请求路径
    ...
    std::string m_body;
    /// 请求头部MAP
    MapType m_headers;//请求字段字段key value
    /// 请求参数MAP
    MapType m_params;//访问参数 key value
    /// 请求Cookie MAP
