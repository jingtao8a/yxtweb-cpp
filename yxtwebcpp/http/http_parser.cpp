 ...
 * @Date: 2022-10-17 16:13:45
 * @LastEditors: yuxintao 1921056015@qq.com
 * @LastEditTime: 2022-10-17 18:58:20
 * @FilePath: /yxtweb-cpp/yxtwebcpp/http/http_parser.cpp
...
static std::shared_ptr<ConfigVar<uint64_t> > g_http_request_buffer_size = 
    Config::Lookup("http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

static std::shared_ptr<ConfigVar<uint64_t> > g_http_request_max_body_size = 
    Config::Lookup("http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

static std::shared_ptr<ConfigVar<uint64_t> > g_http_response_buffer_size =
    Config::Lookup("http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

static std::shared_ptr<ConfigVar<uint64_t> > g_http_response_max_body_size =
...
void on_request_method(void *data, const char *at, size_t length) {
    ...
    if(m == HttpMethod::INVALID_METHOD) {
        YXTWebCpp_LOG_WARN(g_logger) << "invalid http request method: " << std::string(at, length);
...
void on_request_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        YXTWebCpp_LOG_WARN(g_logger) << "invalid http request version: "
...
void on_request_http_field(void *data, const char *field, size_t flen
    ...
    HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
    if(flen == 0) {
        YXTWebCpp_LOG_WARN(g_logger) << "invalid http request field length == 0";
...
void on_response_version(void *data, const char *at, size_t length) {
    ...
    if(strncmp(at, "HTTP/1.1", length) == 0) {
        ...
        YXTWebCpp_LOG_WARN(g_logger) << "invalid http response version: "
            << std::string(at, length);
        parser->setError(1001);
        return;
    }
...
void on_response_http_field(void *data, const char *field, size_t flen
    ...
    HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
    if(flen == 0) {
        YXTWebCpp_LOG_WARN(g_logger) << "invalid http response field length == 0";
        parser->setError(1002);
        return;
    }
    parser->getData()->setHeader(std::string(field, flen), std::string(value, vlen));
