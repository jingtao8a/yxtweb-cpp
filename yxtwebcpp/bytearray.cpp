 ...
 * @LastEditTime: 2022-10-16 15:21:29
 * @FilePath: /yxtweb-cpp/yxtwebcpp/bytearray.cpp
...
void ByteArray::writeFint16 (int16_t value) {
    if(m_endian != YXTWebCpp_BYTE_ORDER) {
...
bool ByteArray::writeToFile(const std::string& name) const {
    ...
    if(!ofs) {
        YXTWebCpp_LOG_ERROR(g_logger) << "writeToFile name=" << name
...
bool ByteArray::readFromFile(const std::string& name) {
    ...
    if(!ifs) {
        YXTWebCpp_LOG_ERROR(g_logger) << "readFromFile name=" << name
            << " error, errno=" << errno << " errstr=" << strerror(errno);
        return false;
    }
